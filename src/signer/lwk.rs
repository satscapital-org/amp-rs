...
impl MnemonicStorage {
    ...
    pub fn with_mnemonics(mnemonics: Vec<String>) -> Result<Self, SignerError> {
        let storage = Self { mnemonic: mnemonics };
        storage.validate()?;
        Ok(storage)
    }
    ...
    pub fn validate(&self) -> Result<(), SignerError> {
        for (index, mnemonic) in self.mnemonic.iter().enumerate() {
            Self::validate_mnemonic_format(mnemonic)
                .map_err(|e| SignerError::InvalidMnemonic(
                    format!("Invalid mnemonic at index {}: {}", index, e)
                ))?;
        }
        Ok(())
    }
    ...
    pub fn validate_mnemonic_format(mnemonic: &str) -> Result<(), String> {
        ...
        if mnemonic.contains("  ") {
            return Err("Multiple consecutive spaces detected, which may indicate empty words".to_string());
        }
        ...
        match words.len() {
            12 | 15 | 18 | 21 | 24 => {},
            _ => return Err(format!("Invalid word count: {}. Expected 12, 15, 18, 21, or 24 words", words.len())),
        }
...
impl MnemonicStorage {
    ...
    pub fn append_mnemonic(&mut self, mnemonic: String) -> Result<usize, SignerError> {
        ...
        Self::validate_mnemonic_format(&mnemonic)
            .map_err(|e| SignerError::InvalidMnemonic(e))?;
    ...
    pub fn get_or_generate_mnemonic_at_index(&mut self, index: usize) -> Result<String, SignerError> {
        ...
        Ok(self.mnemonic.get(index).expect("Mnemonic should exist at index after generation").clone())
    }
    ...
    pub fn read_from_file_path<P: AsRef<Path>>(path: P) -> Result<Self, SignerError> {
        ...
        if !path.exists() {
            tracing::debug!("Mnemonic file {:?} does not exist, returning empty storage", path);
            return Ok(Self::new());
        }
        ...
        // Parse JSON content
        let storage: MnemonicStorage = serde_json::from_str(&contents)
            .map_err(|e| {
        ...
        tracing::info!("Successfully loaded {} mnemonics from {:?}", storage.len(), path);
        Ok(storage)
    }
    ...
    pub fn write_to_file_path<P: AsRef<Path>>(&self, path: P) -> Result<(), SignerError> {
        ...
        fs::write(&temp_path, &contents)
            .map_err(|e| {
                tracing::error!("Failed to write temporary mnemonic file {:?}: {}", temp_path, e);
                SignerError::FileIo(e)
            })?;
        ...
        fs::rename(&temp_path, path)
            .map_err(|e| {
                tracing::error!("Failed to rename temporary file {:?} to {:?}: {}", temp_path, path, e);
...
impl LwkSoftwareSigner {
    ...
    pub fn new(mnemonic_phrase: &str) -> Result<Self, SignerError> {
        ...
        MnemonicStorage::validate_mnemonic_format(mnemonic_phrase)
            .map_err(|e| {
        ...
        let signer = SwSigner::new(mnemonic_phrase, false) // false for testnet/regtest
            .map_err(|e| {
                tracing::error!("Failed to create SwSigner with {}-word mnemonic: {}", 
                               mnemonic.word_count(), e);
                SignerError::Lwk(format!("SwSigner creation failed with {}-word mnemonic: {}", 
                                        mnemonic.word_count(), e))
            })?;

        tracing::info!("Successfully created LwkSoftwareSigner for testnet with {} word mnemonic", 
                      mnemonic.word_count());
    ...
    pub fn generate_new() -> Result<(String, Self), SignerError> {
        ...
        let mnemonic = if let Some(existing_mnemonic) = storage.get_first_mnemonic() {
            ...
            tracing::info!("Found existing mnemonic file with {} mnemonics, using first one", storage.len());
    ...
    pub fn generate_new_indexed(index: usize) -> Result<(String, Self), SignerError> {
        tracing::debug!("Starting generate_new_indexed({}) - loading mnemonic storage", index);
...
impl Signer for LwkSoftwareSigner {
    async fn sign_transaction(&self, unsigned_tx: &str) -> Result<String, SignerError> {
        tracing::debug!("Starting transaction signing process for hex: {}", &unsigned_tx[..std::cmp::min(unsigned_tx.len(), 64)]);
        ...
        if unsigned_tx.trim().is_empty() {
            ...
            return Err(SignerError::InvalidTransaction("Transaction hex cannot be empty".to_string()));
        }
        
        // Input validation - check for reasonable hex length (minimum transaction size)
        if unsigned_tx.len() < 20 { // Minimum reasonable transaction hex length
            tracing::error!("Transaction hex too short: {} characters", unsigned_tx.len());
            return Err(SignerError::InvalidTransaction(
                format!("Transaction hex too short: {} characters (minimum ~20 expected)", unsigned_tx.len())
            ));
        }
        
        // Parse unsigned transaction hex to elements::Transaction
        let tx_bytes = hex::decode(unsigned_tx)
            .map_err(|e| {
                let preview = if unsigned_tx.len() > 40 {
                    format!("{}...{}", &unsigned_tx[..20], &unsigned_tx[unsigned_tx.len()-20..])
                } else {
                    unsigned_tx.to_string()
                };
                tracing::error!("Failed to decode transaction hex (length: {}, preview: '{}'): {}", 
                               unsigned_tx.len(), preview, e);
                SignerError::HexParse(e)
            })?;
        
        tracing::debug!("Successfully decoded hex to {} bytes", tx_bytes.len());
        
        let unsigned_transaction = elements::Transaction::consensus_decode(&tx_bytes[..])
            .map_err(|e| {
                tracing::error!("Failed to deserialize transaction from {} bytes: {}", tx_bytes.len(), e);
                SignerError::InvalidTransaction(format!("Transaction deserialization failed from {} bytes: {}", tx_bytes.len(), e))
            })?;
        
        tracing::debug!("Successfully parsed transaction with {} inputs and {} outputs", 
        ...
        if unsigned_transaction.input.is_empty() {
            ...
            return Err(SignerError::InvalidTransaction("Transaction must have at least one input".to_string()));
        }
        
        if unsigned_transaction.output.is_empty() {
            tracing::error!("Transaction has no outputs");
            return Err(SignerError::InvalidTransaction("Transaction must have at least one output".to_string()));
        }
        ...
        tracing::debug!("Created PSET for signing with {} inputs", pset.inputs().len());
        
        // Validate PSET structure before signing
        if pset.inputs().is_empty() {
            tracing::error!("PSET has no inputs after conversion");
            return Err(SignerError::InvalidTransaction("PSET conversion resulted in no inputs".to_string()));
        }
        
        // Use SwSigner to sign the transaction
        let signed_inputs = self.signer.sign(&mut pset)
            .map_err(|e| {
                tracing::error!("LWK signing operation failed for transaction with {} inputs: {}", pset.inputs().len(), e);
                SignerError::Lwk(format!("Transaction signing failed for {} inputs: {}", pset.inputs().len(), e))
            })?;
        
        tracing::debug!("Successfully signed {} inputs", signed_inputs);
        
        // Extract the signed transaction from PSET
        let signed_transaction = pset.extract_tx()
            .map_err(|e| {
                tracing::error!("Failed to extract signed transaction from PSET: {}", e);
                SignerError::Lwk(format!("Transaction extraction failed after signing {} inputs: {}", signed_inputs, e))
            })?;
        ...
        if signed_hex.is_empty() {
            ...
            return Err(SignerError::InvalidTransaction("Transaction serialization produced empty result".to_string()));
        }
        
        // Add logging for successful signing operations
        tracing::info!("Successfully signed transaction. TXID: {}", signed_transaction.txid());
        tracing::debug!("Signed transaction hex length: {} bytes (original: {} bytes)", 
                       signed_hex.len() / 2, tx_bytes.len());
...
mod tests {
    ...
    fn test_generate_new_behavior_with_existing_file() {
        ...
        let loaded_storage = MnemonicStorage::read_from_file_path(test_file).unwrap();
        assert_eq!(loaded_storage.len(), 1);
        assert_eq!(loaded_storage.get_first_mnemonic().unwrap(), &first_mnemonic);
        ...
        assert_eq!(loaded_storage_again.get_first_mnemonic().unwrap(), &first_mnemonic);
    ...
    fn test_generate_new_indexed_functionality() {
        ...
        for i in 0..all_mnemonics.len() {
            ...
            for j in (i + 1)..all_mnemonics.len() {
                assert_ne!(all_mnemonics[i], all_mnemonics[j], 
                          "Mnemonics at indices {} and {} should be different", i, j);
            }
        }
        ...
        for (i, mnemonic) in all_mnemonics.iter().enumerate() {
            ...
            assert!(signer.is_testnet(), "Signer at index {} should be testnet", i);
        }
        ...
        for i in 0..6 {
            assert_eq!(
                ...
                "Mnemonic at index {} should be preserved after file operations", i
            );
        }
    ...
    fn test_file_writing_and_reading_roundtrip() {
        ...
        assert_eq!(loaded_storage.get_mnemonic(0), original_storage.get_mnemonic(0));
        assert_eq!(loaded_storage.get_mnemonic(1), original_storage.get_mnemonic(1));
    ...
    fn test_file_update_scenario() {
        ...
        let second_mnemonic = "legal winner thank year wave sausage worth useful legal winner thank yellow".to_string();
    ...
    fn test_signer_creation_with_various_mnemonic_inputs() {
        ...
        assert!(result.is_ok(), "Should create signer with valid 12-word mnemonic");
        let signer = result.unwrap();
        assert!(signer.is_testnet(), "Signer should be configured for testnet");

        // Test 2: Another valid 12-word mnemonic
        let valid_12_word_2 = "legal winner thank year wave sausage worth useful legal winner thank yellow";
        let result = LwkSoftwareSigner::new(valid_12_word_2);
        assert!(result.is_ok(), "Should create signer with second valid 12-word mnemonic");
        let signer = result.unwrap();
        assert!(signer.is_testnet(), "Signer should be configured for testnet");

        // Test 3: Third valid 12-word mnemonic
        let valid_12_word_3 = "letter advice cage absurd amount doctor acoustic avoid letter advice cage above";
        let result = LwkSoftwareSigner::new(valid_12_word_3);
        assert!(result.is_ok(), "Should create signer with third valid 12-word mnemonic");
        let signer = result.unwrap();
        assert!(signer.is_testnet(), "Signer should be configured for testnet");
        ...
        match result.unwrap_err() {
            SignerError::InvalidMnemonic(msg) => {
                assert!(msg.contains("word count"), "Error should mention word count issue");
            },
            other => panic!("Expected InvalidMnemonic error, got: {:?}", other),
        }
        ...
        match result.unwrap_err() {
            SignerError::InvalidMnemonic(msg) => {
                assert!(msg.contains("lowercase"), "Error should mention lowercase requirement");
            },
            other => panic!("Expected InvalidMnemonic error, got: {:?}", other),
        }
        ...
        match result.unwrap_err() {
            SignerError::InvalidMnemonic(msg) => {
                assert!(msg.contains("lowercase"), "Error should mention character validation");
            },
            other => panic!("Expected InvalidMnemonic error, got: {:?}", other),
        }
        ...
        assert!(result.is_err(), "Should fail with multiple consecutive spaces");
        match result.unwrap_err() {
            SignerError::InvalidMnemonic(msg) => {
                assert!(msg.contains("consecutive spaces") || msg.contains("empty words"), 
        ...
        match result.unwrap_err() {
            SignerError::InvalidMnemonic(msg) => {
                assert!(msg.contains("BIP39"), "Error should mention BIP39 validation failure");
            },
            other => panic!("Expected InvalidMnemonic error, got: {:?}", other),
        }
    }
    ...
    fn test_network_configuration_validation() {
        ...
        for (i, mnemonic) in test_mnemonics.iter().enumerate() {
            let signer = LwkSoftwareSigner::new(mnemonic).unwrap();
            assert!(signer.is_testnet(), "Signer {} should be configured for testnet", i);
        }

        // Test 2: Verify generated signers are also testnet
        let (_, generated_signer) = LwkSoftwareSigner::generate_new().unwrap();
        assert!(generated_signer.is_testnet(), "Generated signer should be configured for testnet");
        ...
        for index in 0..3 {
            ...
            assert!(indexed_signer.is_testnet(), "Indexed signer {} should be configured for testnet", index);
        }
        ...
        assert_eq!(signer1.is_testnet(), signer2.is_testnet(), 
                  "Network configuration should be consistent across instances");
        assert!(signer1.is_testnet() && signer2.is_testnet(), 
    ...
    async fn test_basic_transaction_signing_flow_with_mock_data() {
        ...
        match result.unwrap_err() {
            SignerError::InvalidTransaction(msg) => {
                assert!(msg.contains("empty"), "Error should mention empty transaction");
            },
            other => panic!("Expected InvalidTransaction error, got: {:?}", other),
        }
        ...
        assert!(result.is_err(), "Should fail with whitespace-only transaction hex");
        match result.unwrap_err() {
            SignerError::InvalidTransaction(msg) => {
                assert!(msg.contains("empty"), "Error should mention empty transaction");
            },
        ...
        assert!(result.is_err(), "Should fail with too short transaction hex");
        match result.unwrap_err() {
            SignerError::InvalidTransaction(msg) => {
                assert!(msg.contains("too short"), "Error should mention transaction too short");
            },
            other => panic!("Expected InvalidTransaction error, got: {:?}", other),
        }
        ...
        let result = signer.sign_transaction(valid_hex_invalid_tx).await;
        assert!(result.is_err(), "Should fail with invalid transaction structure");
        match result.unwrap_err() {
        ...
        assert!(result.is_err(), "Should fail with hex that's too short for transaction");
        ...
        assert!(error_string.contains("Hex parsing failed") || error_string.contains("parsing"), 
               "Error message should provide context about hex parsing failure: {}", error_string);
    }
    ...
    async fn test_thread_safety_and_async_compatibility() {
        ...
        assert_eq!(completed_tasks, vec![0, 1, 2, 3, 4], "All tasks should complete");
        ...
        let file_handles: Vec<_> = (0..3).map(|index| {
            task::spawn(async move {
                ...
                match result {
                    ...
                    Err(e) => {
                        ...
                        match e {
                            SignerError::FileIo(_) | SignerError::Serialization(_) => Ok(actual_index),
                            other => Err(other),
                        }
                    }
                }
        ...
        assert!(successful_file_ops > 0, "At least one concurrent file operation should succeed");
        ...
        let result = signer.sign_transaction("invalid_hex_characters_long_enough_for_test").await;
        assert!(result.is_err(), "Trait object should work correctly");
        match result.unwrap_err() {
            SignerError::HexParse(_) => {}, // Expected
            other => panic!("Expected HexParse error from trait object, got: {:?}", other),
        }
        ...
        let handle = task::spawn(async move {
            ...
            let result = signer.sign_transaction("invalid_hex_characters_long_enough").await;
            assert!(result.is_err());
        });
    ...
    async fn test_signer_error_handling_consistency() {
        ...
        for (input, description) in test_cases {
            ...
            assert!(!debug_msg.is_empty(), "Debug message should not be empty for {}", description);
        }
    }
    ...
    fn test_signer_creation_performance_and_memory() {
        ...
        assert!(elapsed.as_secs() < 5, "Signer creation should be fast, took: {:?}", elapsed);
    ...
    fn test_signer_with_different_mnemonic_languages() {
        ...
        match result {
            Ok(signer) => {
                assert!(signer.is_testnet(), "If signer is created, it should be testnet");
            },
            Err(SignerError::InvalidMnemonic(_)) => {
                // This is also acceptable if the word isn't in the BIP39 wordlist
            },
            Err(other) => {
                panic!("Unexpected error type for potentially invalid words: {:?}", other);
            }
        }
    }
    ...
    fn test_json_file_writing_and_updating() {
        ...
        let mnemonic2 = "legal winner thank year wave sausage worth useful legal winner thank yellow".to_string();
        let mnemonic3 = "letter advice cage absurd amount doctor acoustic avoid letter advice cage above".to_string();
        ...
        overwrite_storage.append_mnemonic(mnemonic1.clone()).unwrap();
        overwrite_storage.append_mnemonic(mnemonic2.clone()).unwrap();
        
        overwrite_storage.write_to_file_path(test_path_overwrite).unwrap();
        let overwritten_loaded = MnemonicStorage::read_from_file_path(test_path_overwrite).unwrap();
        assert_eq!(overwritten_loaded.len(), 2);
        assert_eq!(overwritten_loaded.get_mnemonic_by_index(0).unwrap(), &mnemonic1);
        assert_eq!(overwritten_loaded.get_mnemonic_by_index(1).unwrap(), &mnemonic2);
    ...
    fn test_json_atomic_write_operations() {
        ...
        assert!(Path::new(test_path).exists());
        assert!(!Path::new(temp_path).exists(), "Temporary file should be cleaned up after atomic write");
        ...
        assert!(successful_writes > 0, "At least one concurrent write should succeed");
        ...
        if Path::new(concurrent_path).exists() {
            ...
            assert!(final_storage.is_ok(), "Final file should be valid JSON after concurrent writes");
        }
        ...
        assert!(raw_content.contains("  "), "JSON should be pretty-printed with indentation");
    ...
    fn test_json_file_recovery_scenarios() {
        ...
        fixed_storage.append_mnemonic("legal winner thank year wave sausage worth useful legal winner thank yellow".to_string()).unwrap();
