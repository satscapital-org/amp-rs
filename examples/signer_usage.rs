...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Created signer from mnemonic (testnet: {})", signer1.is_testnet());
    println!();
    ...
    let signers = vec![&signer1, &signer2, &alice_signer, &bob_signer, &charlie_signer];
    ...
    match signer1.sign_transaction(mock_tx_hex).await {
        ...
        Err(SignerError::InvalidTransaction(msg)) => {
            println!("❌ Transaction signing failed (expected with mock data): {}", msg);
        },
        Err(e) => {
...
async fn asset_integration_example() -> Result<(), SignerError> {
    ...
    match asset_distributor.sign_transaction(mock_distribution_tx).await {
        Err(SignerError::InvalidTransaction(_)) => {
