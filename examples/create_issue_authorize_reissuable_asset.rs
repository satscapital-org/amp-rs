//! # Create, Issue, and Authorize Reissuable Asset Example
//!
//! This example demonstrates the complete workflow for creating a new reissuable asset:
//! 1. Derive both confidential and unconfidential addresses from the Elements wallet "amp_elements_wallet_static_for_funding"
//! 2. Issue a reissuable asset with maximum circulation to the confidential address
...
const REISSUANCE_AMOUNT: i64 = 100_000; // 100,000 satoshis for reissuance tokens
...
async fn main() -> Result<(), AmpError> {
    ...
    println!("🏭 Create, Issue, and Authorize Reissuable Asset Example");
    println!("========================================================");
    ...
    // Derive a second address pair for reissuance (must be different from destination)
    let reissuance_unconfidential_address = elements_rpc
        .get_new_address(WALLET_NAME, Some("bech32"))
        .await
        .map_err(|e| {
            AmpError::rpc(format!(
                "Failed to derive reissuance unconfidential address from wallet '{}': {}. Ensure the wallet exists and is loaded.",
                WALLET_NAME, e
            ))
        })?;
    ...
    let asset_name = format!("ReissuableTestAsset_{}", timestamp);
    let asset_ticker = format!("RTA{}", timestamp % 10000);
    ...
    println!("   Is Reissuable: true");
    println!("   Reissuance Amount: {} satoshis", REISSUANCE_AMOUNT);
    println!("   Reissuance Address: {}", reissuance_confidential_address);
    ...
    if let Some(ref reissuance_token_id) = issuance_response.reissuance_token_id {
        println!("   Reissuance Token ID: {}", reissuance_token_id);
    }
    if let Some(reissuance_vout) = issuance_response.reissuance_vout {
        println!("   Reissuance Token VOut: {}", reissuance_vout);
    }
    ...
    println!("   • Reissuance operations");
    println!();
    println!("💡 Use the Asset UUID in other examples:");
    println!("   export TEST_ASSET_UUID={}", issuance_response.asset_uuid);
    if let Some(ref reissuance_token_id) = issuance_response.reissuance_token_id {
        println!();
        println!("💡 Use the Reissuance Token ID for reissuance operations:");
        println!("   export REISSUANCE_TOKEN_ID={}", reissuance_token_id);
    }
