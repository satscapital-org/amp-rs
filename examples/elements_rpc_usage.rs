...
use amp_rs::{AmpError, DistributionResponse, ElementsRpc, TransactionDetail, TxInput, Unspent};
...
async fn main() -> Result<(), AmpError> {
    ...
    println!(
        "✓ TxInput struct created: txid={}, vout={}",
    ...
    println!(
        ...
        tx_detail.txid, tx_detail.confirmations
    );

    let mut map_address_amount = HashMap::new();
    map_address_amount.insert(
        "lq1qq2xvpcvfup5j8zscjq05u2wxxjcyewk7979f9lq".to_string(),
        100.0,
    );
    map_address_amount.insert(
        "lq1qq3xvpcvfup5j8zscjq05u2wxxjcyewk7979f9lq".to_string(),
        50.0,
    );

    let mut map_address_asset = HashMap::new();
    map_address_asset.insert(
        "lq1qq2xvpcvfup5j8zscjq05u2wxxjcyewk7979f9lq".to_string(),
    ...
    println!(
        "✓ DistributionResponse struct created: uuid={}, addresses={}",
    ...
    let _distribution_deserialized: DistributionResponse =
        serde_json::from_str(&distribution_json)?;
    ...
    println!(
        "  • create_raw_transaction() - Build unsigned transactions with Liquid-specific outputs"
    );
