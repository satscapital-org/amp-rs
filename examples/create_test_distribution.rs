...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ready_assignments: Vec<_> = assignments
        .iter()
    ...
    println!(
        "📦 Found {} assignments ready for distribution",
        ...
        ready_assignments
            ...
            .filter_map(|assignment| {
                if let Some(address) = &assignment.receiving_address {
                    ...
                    println!(
                        "⚠️  Skipping assignment {} - no receiving address",
                        assignment.id
                    );
                    None
                }
    ...
    match client
        .create_distribution(asset_uuid, distribution_assignments)
        .await
    {
        Ok(distribution) => {
            ...
            for assignment in updated_assignments {
                if let Some(dist_uuid) = &assignment.distribution_uuid {
                    println!(
                        "   Assignment {} now linked to distribution {}",
