...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, url_var, user_var, pass_var) in [
        ("Local", "ELEMENTS_RPC_URL", "ELEMENTS_RPC_USER", "ELEMENTS_RPC_PASSWORD"),
        ("Cloud", "CLOUD_ELEMENTS_RPC_URL", "CLOUD_ELEMENTS_RPC_USER", "CLOUD_ELEMENTS_RPC_PASSWORD"),
    ] {
        ...
        println!("Wallet type: {}", if is_descriptor { "Descriptor" } else { "Legacy" });
        ...
        if is_descriptor {
            ...
            match rpc.list_descriptors(WALLET_NAME, Some(false)).await {
                Ok(descriptors) => {
                    ...
                    let has_private = descriptors.iter().any(|d| d.contains("prv") || d.contains("xprv"));
                    println!("Has private keys: {}", if has_private { "❌ NO (watch-only)" } else { "⚠️  Unknown" });
                    
                    // Try to list with private keys explicitly
                    match rpc.list_descriptors(WALLET_NAME, Some(true)).await {
                        Ok(private_descriptors) => {
                            let actually_has_private = private_descriptors.iter().any(|d| d.contains("prv") || d.contains("xprv"));
                            println!("Private key check: {}", if actually_has_private { "✅ YES" } else { "❌ NO (watch-only)" });
                        }
                        Err(e) => {
            ...
            match rpc.get_new_address(WALLET_NAME, None).await {
                Ok(address) => {
                    match rpc.dump_private_key(WALLET_NAME, &address).await {
