use amp_rs::client::{ApiClient, TokenEnvironment, MockTokenStrategy, TokenStrategy};
...
async fn test_api_client_with_mock_strategy() {
    ...
    let token2 = client.get_token().await.expect("Failed to get mock token again");
    assert_eq!(token2, mock_token);
    
    // Verify clear_token works (should be no-op for mock)
    client.clear_token().await.expect("Failed to clear mock token");
    
    // Token should still be available after clear (mock behavior)
    let token3 = client.get_token().await.expect("Failed to get mock token after clear");
...
async fn test_strategy_creation() {
    ...
    let token = mock_strategy.get_token().await.expect("Failed to get token from mock strategy");
    ...
    let auto_token = auto_strategy.get_token().await.expect("Failed to get token from auto strategy");
...
async fn test_api_client_strategy_methods() {
    ...
    let refreshed_token = client.force_refresh().await.expect("Failed to force refresh");
...
async fn test_mock_strategy_isolation() {
    ...
    strategy1.clear_token().await.expect("Failed to clear token1");
    
    let token1_after_clear = strategy1.get_token().await.expect("Failed to get token1 after clear");
    let token2_after_clear = strategy2.get_token().await.expect("Failed to get token2 after clear");
