...
fn test_token_persistence_environment_detection() {
    ...
    if let Some(val) = original_tests { env::set_var("AMP_TESTS", val); }
    if let Some(val) = original_persistence { env::set_var("AMP_TOKEN_PERSISTENCE", val); }
    if let Some(val) = original_username { env::set_var("AMP_USERNAME", val); }
    if let Some(val) = original_password { env::set_var("AMP_PASSWORD", val); }
}