use amp_rs::{ElementsRpc, TxInput, Unspent, AmpError};
        ...
        if let Some(ref expected) = self.expected_input {
            if unsigned_tx != expected {
                return Err(SignerError::InvalidTransaction(format!(
                    "Expected transaction '{}', got '{}'", expected, unsigned_tx
                )));
            }
        }
        
        if !self.should_succeed {
            return Err(SignerError::Lwk("Mock signing failure for testing".to_string()));
        }
...
fn create_gettransaction_mock(server: &MockServer, txid: &str, confirmations: u32, blockheight: Option<u64>) {
    server.mock(|when, then| {
        when.method(POST)
            .path("/")
            .json_body(json!({
...
fn create_createrawtransaction_mock(server: &MockServer, expected_inputs: Vec<TxInput>, expected_outputs: HashMap<String, f64>, expected_assets: HashMap<String, String>) {
    server.mock(|when, then| {
        when.method(POST)
            .path("/")
            .json_body(json!({
...
async fn test_liquid_specific_transaction_format() {
    ...
    let mut address_amounts = HashMap::new();
    address_amounts.insert("lq1qq2xvpcvfup5j8zscjq05u2wxxjcyewk7979f9lq".to_string(), 50.0);
    
    let mut expected_outputs = HashMap::new();
    expected_outputs.insert("lq1qq2xvpcvfup5j8zscjq05u2wxxjcyewk7979f9lq".to_string(), 50.0);
    expected_outputs.insert("address_0".to_string(), 49.0); // Change
    
    let mut expected_assets = HashMap::new();
    expected_assets.insert("lq1qq2xvpcvfup5j8zscjq05u2wxxjcyewk7979f9lq".to_string(), asset_id.to_string());
    expected_assets.insert("address_0".to_string(), asset_id.to_string());
...
async fn test_transaction_construction_zero_amount_distribution() {
    ...
    let error = result.unwrap_err();
    println!("Actual error: {}", error);
    assert!(error.to_string().contains("Total distribution amount must be greater than zero"));
}
...
async fn test_signer_validation_comprehensive() {
    ...
    for (invalid_tx, expected_error) in test_cases {
        ...
        assert!(result.is_err(), "Expected error for input: '{}'", invalid_tx);
        let error = result.unwrap_err();
        assert!(
        ...
        assert_eq!(mock_signer.call_count(), 0, "Signer should not be called for invalid input: '{}'", invalid_tx);
    }
}
...
async fn test_confirmation_polling_timeout() {
    ...
    assert!(error.to_string().contains("Timeout waiting for confirmations"));
...
async fn test_confirmation_polling_custom_minimum_confirmations() {
    ...
    let result = rpc.wait_for_confirmations_with_interval(txid, Some(6), Some(0), Some(1)).await;
...
async fn test_change_data_collection_rpc_error() {
    ...
    assert!(error.to_string().contains("Failed to query unspent outputs"));
}
...
async fn test_change_data_formatting_for_api() {
    ...
    assert_eq!(change_utxo.address, "lq1qq2xvpcvfup5j8zscjq05u2wxxjcyewk7979f9lq");
    assert!(change_utxo.spendable);
    assert_eq!(change_utxo.confirmations, Some(2));
    assert_eq!(change_utxo.scriptpubkey, Some("76a914abcdef1234567890abcdef1234567890abcdef88ac".to_string()));
...
async fn test_collect_change_data_integration() {
    ...
    assert_eq!(change_utxo_1.address, "lq1qq2xvpcvfup5j8zscjq05u2wxxjcyewk7979f9lq");
    
    let change_utxo_2 = change_utxos.iter().find(|u| u.vout == 2).unwrap();
    assert_eq!(change_utxo_2.amount, 10.0);
    assert_eq!(change_utxo_2.address, "lq1qq3xvpcvfup5j8zscjq05u2wxxjcyewk7979f9lq");
}
